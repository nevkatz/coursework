...
pid_t start_command(command* c, pid_t pgid) {
  ...
  //pgid = pid;
     ...
     */
  if((pgid != 0) && (pid != 0)){
    //printf("seeting rest child pgid\n");
    setpgid(pid, pgid);
  }
  if(pgid == 0) {  
    if (pid == 0){
      //printf("setting first child pgid\n");
      setpgid(0, 0);
    }
    else{
      //printf("setting parent return value\n");
      setpgid(pid, pid);
    }
  }
...
void interrupt_handler (int sig) 
{
  ...
  fflush(stdout);
  printf("interrupt handler called. kill_process=%i\n", kill_process);
}
