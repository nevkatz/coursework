...
I'll leave it to you guys to figure out how to generalize that into your run_list loop.
 
***********************************************
In lecture he talks about sth =>
set up fd pipeline

put fg pipeline in fg

if(shell got ctrl c) kill fg pipeline

wait for fg pipeline
  ...
  while(c)
      ...
      if(((c->connectivity == 1) && exit_status) || ((c->connectivity == 2) && !exit_status))
	{
     ...
     // pid_t pid = -1;
      ...
      // I don't know how else to get the pgid which I assume
      // we need for set_foreground.
...
int main(int argc, char* argv[]) {
	...
	needprompt = 0;
      ...
      /* if (signal(SIGINT, handler) == SIG_ERR)
          ...
          // what handler is needed for handle_signal?
         int h = handle_signal(SIGINT, SIG_DFL);
