...
void run_list(command* c) {
   ...
   /* printf("yo yo%i\n", c->argc);
    ...
    else if (strcmp(c->argv[0], str3) == 0)
    {
        const char* file = (const char*) c->argv[1];
        char **argv = &(c->argv[1]);
        execvp(file, argv);
    }
...
int main(int argc, char* argv[]) {
    ...
    // declare position in the buffer. 
    int bufpos = 0;
    ...
    while (!feof(command_file)) {
        ...
        // determine position in the buffer based on length of string.
        bufpos = strlen(buf);
