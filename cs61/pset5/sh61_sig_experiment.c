...
//       - Call `set_foreground(pgid)` before waiting for the pipeline. (Other case?)
//       - Call `set_foreground(0)` once the pipeline is complete. (Done right?)
//       - Cancel the list when you detect interruption. (How to cancel?)
...
void run_list(command* c) {
  ...
  while(c)
      ...
      if(((c->connectivity == 1) && exit_status) || ((c->connectivity == 2) && !exit_status))
	{
      ...
      // how to get pgid? we should probably get it here. 
      ...
      if(((next && (next->connectivity != 0)) || (next == NULL)) && c->wait)
	  {
	    ...
	    // based on the directions above, I think we may 
	    //  place our call here. 
	    
	    // if (signal(SIGINT, handler) == SIG_ERR)
	       // kill(-pid, signal) goes here, possibly
