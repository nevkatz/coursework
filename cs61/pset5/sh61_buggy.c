...
int print_history()
{
 ...
 { 
    ...
    printf("%s\n", cur_node->str);
    string_node* next = cur_node->next;
    printf("next: %s", next->str);
...
int push_to_history (char buf[BUFSIZ])
{
  ...
  {
    // shift the head of the list by 1
  ...
  if (history_list->count == 1) {
    ...
    printf("count: %i\n", history_list->count);
   // repoint last to a new memory block.
    if (history_list->count == 2) 
    {
       history_list->last = malloc(sizeof(string_node));
       string_node* head = history_list->first;
       head->next = new_node;
    }
...
void run_list(command* c) {
  ...
  while(c)
   ...
   /*   if (strcmp(c->argv[0], "his") == 0) // convert to caps? 
      {
        print_history();
        break;
      }*/
...
int main(int argc, char* argv[]) {
    ...
    // allocate space for first node of history list
