...
void http_receive_response_body(http_connection* conn) {
    ...
    if (conn->state < 0)
        return;
   // printf("response body after return with fd: %i\n", conn->fd);
   ...
   // printf("while loop\n");
    ...
    // null-terminate body
    conn->buf[conn->len] = 0;
   // printf("end of receive function\n");
}
...
{     
  ...
  unsigned long current_time = timestamp() * 1000000; 
  
  // printf("current time: %i\n", (int) current_time);
  
  unsigned long wait = resume_time - current_time;
  
  // printf("resume time: %lu wait: %lu \n", resume_time, wait);
  
  if(resume_time > 0 ) usleep(wait);
...
void* pong_thread(void* threadarg) {
    ...
    if (result > 0)
    {
      ...
      printf("stop detected. result: %i\n", (int) result);
     
 
      
     
      //unsigned long result_microns = (unsigned long) result *1000;
      
      printf("result in microns: %lu\n", result_microns);
    
      
      unsigned long current_time = (unsigned long) timestamp() * 1000000;
      
        printf("current time: %lu\n",current_time);
      resume_time = current_time + result_microns;

       printf("resume_time after: %lu\n",  resume_time);
