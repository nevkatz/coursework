...
{     
 ...
 // printf("resume time: %0.0f wait: %f in microns: %0.0f \n", resume_time, wait, (double)  wait_microns);
...
static int http_process_response_headers(http_connection* conn) {
   ...
   // strcpy(message, &conn->buf[17]);
   // printf("message: %s^^^^\n^^^^", message);
  //  if (strcmp(message,"X-Very-Long-Header: ") == 0) printf("LONG!!!!!!\n");
   // printf("buf: %s\n", conn->buf);
