...
{
  pushing++;
  printf("my_conn_list: %p\n", (void*) my_conn_list);
  printf("1) my_conn_list->first: %p\n", (void*) my_conn_list->first);
  printf("conn: %p\n", (void*) conn);
  printf("count: %i\n", my_conn_list->count);
  if (conn)
  {
    ...
    {  
     ...
     printf("2) my_conn_list->first: %p\n", (void*) my_conn_list->first);
    my_conn_list->first = (http_connection*) conn;
...
http_connection* http_connect(const struct addrinfo* ai) {
    ...
    if (my_conn_list->count > 0)
    {
      printf("recycle connection / count: %i\n", my_conn_list->count);
    ...
    // construct an http_connection object for this connection
      ...
      printf("NEW connection. list count=%i\n", my_conn_list->count);
...
void* pong_thread(void* threadarg) {
    ...
    //printf("attempt connection with %d,%d\n", pa.x, pa.y);
    char url[256];
    ...
    n_requests++;
   // printf("sent request outside loop - req=%i\n",n_requests);
    ...
    if (conn->status_code != 200)
    {
      ...
      {
        connection_down = 1;
        // printf("close connection with %d,%d\n",pa.x,pa.y);
        // close that connection (to free its resources) 
        
        printf("closing %p in while loop\n", (void*) conn);
        ...
        http_send_request(conn, url);
        n_requests++;
       //  printf("sent request in loop -- %d,%d -- req=%i\n", pa.x, pa.y, n_requests);
        // send the request
    ...
    n_requests = 0;
     // printf("SUCCESS with (%d,%d)active connections: %i\n", pa.x, pa.y, n_connection_threads);
    http_receive_response_body(conn);
    ...
    printf("was closing %p at end\n", (void*) conn);
    
    // are we supposed to not close the connection here? 
    // pop_from_list(conn);
    ...
    // we may need a check list function here. 
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        while (n_connection_threads >= MAX_THREADS || n_requests > 0 || connection_down == 1)
          if (n_connection_threads >= MAX_THREADS || n_requests > 0 || connection_down == 1)
...
static int http_check_response_body(http_connection* conn) {
  //  printf("check response body pushing: %i conn: %p state: %i\n", pushing, (void*) conn, conn->state);
        ...
        && conn->len >= conn->content_length)
    {
        ...
        printf("about to push with %p...\n", (void*) conn);
        
        while (pushing > 0) usleep(1);
  //      printf("after while with %p\n...\n", (void*) conn);
