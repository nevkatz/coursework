...
void* pong_thread(void* threadarg) {
   ...
   // printf("pong thread start\n");
    ...
    // new
	  ...
	  long wait = wait_time - current_time;
	  printf("wait %lu \n", wait);
	  if(wait > 0)
	    usleep(wait);
	//  wait_time = 0;
    ...
    // check for stop. 
    if(strcmp(secondPart, "STOP") == 0) {
      int time;
   //   printf("stop detected\n");
      // get the time
      sscanf(firstPart, "+%d", &time);
      
      // declare a new timeval. 
      struct timeval now;
      
      // get the time of day.
      gettimeofday(&now, NULL);
      
      // compute wait time (what is this doing?)
      wait_time = now.tv_sec*1000000 + now.tv_usec;
      
      // determine based on time
      wait_time = wait_time + time * 1000;
      
      printf("stop detected -- wait time: %i\n", wait_time);
    }
