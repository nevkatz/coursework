...
void* pong_thread(void* threadarg) {
    ...
    while ((pa.index - index_sent) > 1 || sending == 1 || (conn->state != 0 && conn->state != -1)) usleep(1);
    ...
    if (conn->status_code != 200)
    {
      ...
      while (conn->state == HTTP_BROKEN || conn->status_code == -1) 
      {
       // printf("failed\n");
        ...
        while (n > 1 || sending == 1 ||
          (conn->state != 0 && conn->state != -1)) usleep(1);
    ...
    while (1) {
       ...
       // printf("lock: %.3f\n", elapsed());
        pthread_mutex_lock(&mutex);
          if (n_connection_threads >= MAX_THREADS || n_requests > 0)
