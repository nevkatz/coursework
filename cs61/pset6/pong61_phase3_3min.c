...
// sleep constants
#define DIS_CONST 100000 // 10000
#define DIS_MAX 20000000 
...
void* pong_thread(void* threadarg) {
   ...
   //  printf("out of loop - request # %i\n", n_requests);
    ...
    //    {
       ...
       /* while (n_requests >= MAX_REQ && conn->status_code == -1 ) 
        {
            printf("wait...req=%i\n", n_requests);
            usleep(1);
        }*/
        
       // printf("in loop - request # %i\n", n_requests);
        http_send_request(conn, url);
        n_requests++;
        ...
        disconnects++;
   //     }
   //     else { printf("maxed out -- %i requests\n", n_requests); }
      } 
      connection_down = 0;
     // n_requests = 0;
