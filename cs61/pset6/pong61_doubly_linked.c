...
// Make it a singly linked list that operates as a stack. You may not even need a struct for the list itself, just a done_head.  
...
// Linked List
   // This is a linked list of connections that are DONE and ready to use.
...
http_connection* traverse_list ()
{
  http_connection* conn = my_conn_last;
   ...
   // printf("sent request outside loop - req=%i\n",n_requests);
    ...
    if (conn->status_code != 200)
    {
      ...
      while (conn->state == HTTP_BROKEN || conn->status_code == -1) 
      {
        ...
        http_send_request(conn, url);
        n_requests++;
        //  printf("sent request in loop -- %d,%d -- r=%i\n", pa.x, pa.y, n_requests);
        // send the request
