...
// start of done linked list
http_connection* list_start;
...
int push_to_list (http_connection* conn)
{
   ...
   http_connection* temp = conn;
  

  if (list_start) 
      conn->next = list_start;
   else
      conn->next = NULL;
...
// remove from list
http_connection* pop_from_list ()
{

   http_connection* myconn = list_start;
   
    if (myconn->next)  
      list_start = myconn->next;
    else
      list_start = NULL;
...
http_connection* http_connect(const struct addrinfo* ai) {
    ...
    // either get a new connection or recycle one
    if (list_start) 
      conn = pop_from_list();
    else
