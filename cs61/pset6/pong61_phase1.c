...
#define DIS_CONST 10000

#define DIS_MAX 2000000    
   
/*
In Phase 2, the server delays its responses. It will send you the full header for its response, but delay the body.

 Since in the handout code the pong loop waits for one request to complete before sending the next, the pong ball will move extremely slowly in Phase 2. Too slowly, in fact: the server enforces a minimum ball speed, and when your code is slower than that speed, you’ll see some black marks on the display.
...
To support delay, your pong61 must handle multiple concurrent connections to the server. Now, the main thread should sometimes spawn a new thread before the previous thread exited!
...
/*
 *  NK: STATE gets set to HTTP_BROKEN
 */
...
void http_receive_response_headers(http_connection* conn) {
    ...
    /*
     * Do I set the Step 2 conditional inside or outside the loop? 
     *
     */
...
void http_receive_response_body(http_connection* conn) {
    ...
    while (http_check_response_body(conn)) {
        ...
        if (nr == 0)
            ...
            perror("read");
            exit(1);
        } else if (nr != -1) // increase length of connection
            conn->len += nr;
    }
...
// pong_args stores ONE x-y location of our ball.
...
void* pong_thread(void* threadarg) {
    ...
    // this thread can be reclaimed when the thread terminates...
    // but it cannot be reaped or killed by other threads. 
    // its memory resources are freed automatically when thread terminates. 
    pthread_detach(pthread_self());

    // Copy thread arguments onto our stack.
    // pong_args is an x-y pair of coordinates, pointed to by threadarg. 
    pong_args pa = *((pong_args*) threadarg);
    
    
    // declare a URL char array on the stack.
    char url[256];
    
    // sends over the coordinates as a query string. 
    snprintf(url, sizeof(url), "move?x=%d&y=%d&style=on",
             pa.x, pa.y);
    
    // connect at the pong address, which is a global variable. 
    ...
    // I think we add code here....
    while (conn->state == HTTP_BROKEN || conn->status_code == -1) 
    {
     
      // close that connection (to free its resources) 
      http_close(conn);

      
      //wait for 2^N * K seconds with max backoff of dis_max  
      // 2^n operation
      int multiplier = pow(2,disconnects);
